...
class ConvElm(nn.Module):
    ...
    class for network
    '''
    def __init__(self, feat_len, win_len, conv1_ch_mul, conv1_kernel_size,     l2_parm, model_path):
        ...
        flatten_width = self.feat_len*self.conv1_ch_mul * round(self.win_len)

        print ("flatten_width", flatten_width)

        # self.fc1 = nn.Linear(flatten_width, 50)
        self.fc2 = nn.Linear(flatten_width, 1, bias=False)
...
def train_loop(train_sample_array, train_label_array, model, loss_fn, optimizer):
    ...
    hiddenOut = model.forwardToHidden(train_sample_array)

    optimizer.train(inputs=hiddenOut, targets=train_label_array)
