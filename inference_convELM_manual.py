...
def main():
    ...
    convELM_model = Net(feat_len, win_len, conv1_ch_mul, conv1_kernel_size, conv2_ch_mul, conv2_kernel_size, conv3_ch_mul, conv3_kernel_size, l2_parm, model_temp_path)
    print("convELM_model", convELM_model)

    validation, model = train_net(convELM_model, train_sample_array, train_label_array, val_sample_array,
                                val_label_array, l2_parm, device)
    ...
    sample_array = np.concatenate(test_units_samples_lst)
    label_array = np.concatenate(test_units_labels_lst)
    print ("samples are aggregated")
    ...
    test_sample_array = array_tensorlst_data(sample_array, bs)
    test_label_array = array_tensorlst_label(label_array, bs)


    test_rmse, test_score, test_output, test_target = test_net(model, test_sample_array, test_label_array)
